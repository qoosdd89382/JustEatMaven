...
public class AdminChatroomJedisDAO implements AdminChatroomDAOInterface {
	...
	public boolean insert(AdminChatroomVO adminChatroom) {
		...
		try {
			...
			StringBuilder key = new StringBuilder().append(adminChatroom.getAccountID()).append(":")
					.append(adminChatroom.getAdminID());
			// 放
...
//
//		if (pool != null) {
		JedisUtil.shutdownJedisPool(); // 關閉連線池? servlet destroy() 用?
//		}
	}
		...
		if (pool == null) {
			synchronized (JedisUtil.class) {
				if (pool == null) {
					...
					pool = new JedisPool(config, "justeat.chickenkiller.com", 6379);
				}
			}
		}
		return pool;
